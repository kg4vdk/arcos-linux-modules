...
if [ -f $QRV_PROFILE_DIR/${QRV_PROFILE}/$MODULE/qsstv_9.0_${QRV_PROFILE}.conf ]; then
	mv $QRV_PROFILE_DIR/${QRV_PROFILE}/$MODULE/qsstv_9.0_${QRV_PROFILE}.conf  $QRV_PROFILE_DIR/${QRV_PROFILE}/$MODULE/qsstv_9.0_${QRV_PROFILE}_$(date +"%F_%H%M").conf
	cp $HOME/.config/ON4QZ/qsstv_9.0.conf $QRV_PROFILE_DIR/${QRV_PROFILE}/$MODULE/qsstv_9.0_${QRV_PROFILE}.conf 
else
...
if echo "${QRV_PROFILE}" | grep "|" > /dev/null; then
	if qsstv_save; then
		notify-send --icon=qsstv-custom-icon "QSSTV" "Configuration saved!"
	else
		notify-send --icon=error "QSSTV" "Error saving configuration!"
	fi
fi
