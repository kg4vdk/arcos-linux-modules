...
# Set system calendar revision to timestamp
sed -i "s/Revision=.*$/Revision=$TIMESTAMP/" $SAVE_DIR/sources/arcOS.source

# Set user calendar revision to timestamp
sed -i "s/Revision=.*$/Revision=$TIMESTAMP/" $SAVE_DIR/sources/user.source
